...
EVT_WDF_DEVICE_CONTEXT_CLEANUP virt2phys_driver_EvtContextCleanup;
...
virt2phys_driver_EvtDeviceAdd(WDFDRIVER driver, PWDFDEVICE_INIT init)
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("WdfDeviceCreateDeviceInterface() failed, status=%08x\n", status));
        return status;
    }
        ...
        IN WDFDEVICE device, IN WDFREQUEST request)
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("WdfRequestRetrieveInputBuffer() failed, status=%08x\n", status));
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("WdfRequestRetrieveOutputBuffer() failed, status=%08x\n", status));
        WdfRequestComplete(request, status);
        return;
    }
    ...
    *phys = MmGetPhysicalAddress(*virt);
    ...
    WdfRequestCompleteWithInformation(request, STATUS_SUCCESS, sizeof(*phys));
}

/* Mandatory callback for all drivers. */
VOID
virt2phys_driver_EvtContextCleanup(IN WDFOBJECT driver)
{
    UNREFERENCED_PARAMETER(driver);
