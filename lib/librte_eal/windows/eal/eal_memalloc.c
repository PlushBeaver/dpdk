...
alloc_seg_walk(const struct rte_memseg_list *msl, void *arg)
{
	 ...
	 * another one.
	 */
	if (EAL_LOCK_VALID(wa->hi->lock_descriptor) &&
			!internal_config.in_memory) {
		if (!WaitForSingleObject(wa->hi->lock_descriptor, INFINITE)) {
			RTE_LOG_SYSTEM_ERROR("WaitForSingleObject()");
			return -1;
		}
		locked = true;
	}
	...
	for (i = 0; i < need; i++, cur_idx++) {
		...
		if (alloc_seg(cur, map_addr, wa->socket, wa->hi)) {
			...
			if (locked) {
				ReleaseMutex(wa->hi->lock_descriptor);
			}
			return -1;
		}
		if (wa->ms) {
